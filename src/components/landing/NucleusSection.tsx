...
export default function NucleusSection() {
  const t = useTranslations("nucleusSection");
  
  // Split the title to apply different styling to the second part
  const titleParts = t('title').split('|');
  
  return (
    <section id="nucleos" className="bg-dark-900 py-20 relative overflow-hidden">
      {/* Background Gradient */}
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-gotham-black text-white mb-6">
            {titleParts[0]} <span className="text-primary-500">{titleParts[1]}</span>
          </h2>
        </div>
          ...
          <div id="sales-nucleus" className="relative overflow-hidden bg-gradient-to-br from-dark-700 to-dark-800 border border-dark-600 rounded-2xl p-0 shadow-2xl group hover:border-primary-500/50 transition-all duration-300 flex flex-col h-full">
            ...
            <div className="p-8 flex-1 flex flex-col">
              <div className="flex items-center mb-6">
                ...
                <h3 className="text-2xl font-gotham-black text-white group-hover:text-primary-400 transition-colors duration-300">{t('salesCard.title')}</h3>
              </div>
              
              <p className="text-gray-300 mb-8 leading-relaxed h-[60px]">
                {t('salesCard.description')}
              </p>
              
              {/* Feature blocks - redesigned */}
              <div className="flex flex-col space-y-4 mb-8">
                {t.raw('salesCard.features').map((feature: any, index: number) => (
                  <div key={index} className="bg-dark-600/30 backdrop-blur-sm p-5 rounded-xl border border-dark-500/50 hover:border-primary-500/30 transition-all duration-300">
          ...
          <div id="tech-nucleus" className="relative overflow-hidden bg-gradient-to-br from-dark-700 to-dark-800 border border-dark-600 rounded-2xl p-0 shadow-2xl group hover:border-primary-500/50 transition-all duration-300 flex flex-col h-full">
            ...
            <div className="p-8 flex-1 flex flex-col">
              <div className="flex items-center mb-6">
                ...
                <h3 className="text-2xl font-gotham-black text-white group-hover:text-primary-400 transition-colors duration-300">{t('techCard.title')}</h3>
              </div>
              
              <p className="text-gray-300 mb-8 leading-relaxed h-[60px]">
                {t('techCard.description')}
              </p>
              
              {/* Feature blocks - redesigned */}
              <div className="flex flex-col space-y-4 mb-8">
                {t.raw('techCard.features').map((feature: any, index: number) => (
                  <div key={index} className="bg-dark-600/30 backdrop-blur-sm p-5 rounded-xl border border-dark-500/50 hover:border-primary-500/30 transition-all duration-300">
                    <div className="flex items-center mb-3">
                      <div className="bg-primary-500/20 p-2 rounded-lg mr-3">
                        <svg className="w-5 h-5 text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
